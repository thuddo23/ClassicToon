...
class GetTrendingBooksUseCaseTest {
    ...
    fun setUp() {
        ...
        useCase = GetTrendingBooksUseCase(repository)
    }
    ...
    fun `invoke should return trending books from repository`() = runTest {
        ...
        assertTrue("Should return trending books", result.isNotEmpty())
        ...
        println("Retrieved ${result.size} trending books from use case:")
        result.forEach { book ->
    ...
    fun `invoke should use default limit when not specified`() = runTest {
        ...
        // Then
        assertNotNull(result)
        assertTrue("Should return trending books with default limit", result.isNotEmpty())
        
        // Log the results for debugging
        println("Retrieved ${result.size} trending books with default limit from use case")
    }
    ...
    @Test
    fun `invoke should handle custom limit values`() = runTest {
        // When
        val customLimit = 10
        val result = useCase(limit = customLimit)
        
        // Then
        assertNotNull(result)
        assertTrue("Should return books with custom limit", result.isNotEmpty())
        
        // Log the results for debugging
        println("Retrieved ${result.size} trending books with custom limit $customLimit from use case")
    }
}
