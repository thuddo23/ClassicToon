...
class MockApiService @Inject constructor() {
    ...
    // Feed endpoints
    suspend fun getTrendingBooks(limit: Int = 20): BookListResponse {
        delay(500)
        val items = mockBooks.take(limit).map { book ->
            FeedBookItem(
                ...
                badge = if (book.tags.contains("chapter")) "New Update" else null,
    ...
    suspend fun getNewestBooks(limit: Int = 20): FeedResponse {
        delay(350)
    ...
    suspend fun getBooks(
        ...
        if (!searchQuery.isNullOrBlank()) {
            filteredBooks = filteredBooks.filter { book ->
                ...
                book.authors.any { it.contains(searchQuery, ignoreCase = true) } ||
    ...
    suspend fun getBookDetail(bookId: String): RemoteBookDetailResponse? {
        delay(300)
    ...
    private fun generateChapteredHtmlContent(book: RemoteBookResponse?): String {
    ...
    private fun generateSimpleHtmlContent(book: RemoteBookResponse?): String {
        val title = book?.title ?: "Unknown Book"
        val author = book?.authors?.firstOrNull() ?: "Unknown Author"
